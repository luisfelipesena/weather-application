...
import { City } from '../../api/city/types';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-blue-300 p-8">
      <Card className="max-w-md mx-auto bg-white/90">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">City Search</CardTitle>
        </CardHeader>
        <CardContent>
          <AutocompleteInput
            value={input}
          ...
          <Button className="w-full mt-4" disabled={isLoading} onClick={handleGetWeather}>
            {isLoading ? 'Searching...' : 'Get Weather'}
          </Button>

          <Button className="w-full mt-2" disabled={isGeolocating} onClick={handleGeolocation}>
