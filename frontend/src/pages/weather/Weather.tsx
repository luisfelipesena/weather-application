...
import {  Droplets, Wind, Eye, Cloud } from "lucide-react"
...
export const WeatherDashboard = () => {
    ...
    const getBackgroundClass = (condition: string) => {
      ...
      if (lowerCondition.includes("sun") || lowerCondition.includes("clear")) return "bg-gradient-to-br from-yellow-300 to-orange-500"
      if (lowerCondition.includes("cloud")) return "bg-gradient-to-br from-gray-300 to-gray-500"
      if (lowerCondition.includes("rain")) return "bg-gradient-to-br from-blue-300 to-blue-500"
      return "bg-gradient-to-br from-purple-300 to-purple-500"
    }

    if (isLoading) return <div className="flex justify-center items-center h-screen">Loading...</div>

    if (!weather) return <div className="flex justify-center items-center h-screen">No weather data</div>

    return (
      <div className={`min-h-screen ${getBackgroundClass(weather.condition)} p-8`}>
        <Card className="max-w-md mx-auto bg-white/80">
          <CardHeader>
            <div className="flex justify-start mb-4">
              <Link to="/city" className="text-blue-600 hover:text-blue-800 transition-colors duration-200">
                Back to City Search
              </Link>
            </div>
